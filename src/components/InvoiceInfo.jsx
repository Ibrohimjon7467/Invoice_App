...
function InvoiceInfo({ onDelete }) {
    ...
    return (
        <div>
            {invoice ? <motion.div key='invoice-info' initial={{ x: 0 }} animate={{ x: 0 }} exit={{ x: '200%' }} transition={{ duration: 0.5 }} className='dark:bg-[#141625] mx-auto duration-300 min-h-screen bg-[#f8f8fb] py-[34px] px-2 md:px-8 lg:px-12 max-w-3xl lg:py-[72px] '>
                <div>
                    ...
                    <div className='mt-8 rounded-lg w-full flex items-center justify-between px-6 py-6 bg-white dark:bg-[#1e2139]'>
                        ...
                        <div className=' md:block hidden'>
                            ...
                            {invoice.status === 'pending' && <button onClick={onMakePaidClick} className='ml-3 text-center text-white bg-[#7c5dfa] hover:opacity-80 p-3 px-7 rounded-full'>Make as Paid</button>}
                        </div>
                    </div>
