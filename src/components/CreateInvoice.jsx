...
function CreateInvoice({ setOpenCreateInvoice, invoice, type }) {
    ...
    return (
        <div onClick={(e) => {
            ...
            <motion.div key='createInvoice-sidebar' initial={{ x: -500, opacity: 0 }} animate={{ opacity: 1, x: 0, transition: { type: 'spring', stiffness: 300, damping: 40, duration: .4 } }} exit={{ x: -700, transition: { duration: .2 } }}
                ...
                <div className=' overflow-y-scroll scrollbar-hide my-14'>
                    ...
                    <div className='grid grid-cols-3 mx-1   space-y-4 '>
                        ...
                        <div className='flex flex-col mr-4 col-span-1'>
                            ...
                            <input type='text' value={clientPostCode} onChange={(e) => setClientPostCode(e.target.value)} className={`dark:bg-[#1e2139] py-2 px-4 border-[.2px] rounded-lg  focus:outline-purple-400 border-gray-300 focus:outline-none ${isValidatorActive && !validateClientPostCode(clientPostCode) && 'border-red-500 dark:border-red-500 outline-red-500 border-2'}dark:border-gray-800`} />
                        </div>
                        <div className='flex flex-col col-span-1'>
                            <label className='text-gray-400 font-light'>Country</label>
                            <input type='text' value={clientCountry} onChange={(e) => setClientCountry(e.target.value)} className={` dark:bg-[#1e2139] py-2 px-4 border-[.2px] rounded-lg  focus:outline-purple-400 border-gray-300 focus:outline-none ${isValidatorActive && !validateClientCountry(clientCountry) && 'border-red-500 dark:border-red-500 outline-red-500 border-2'}   dark:border-gray-800`} />
                        </div>
                    </div>
                    <div className='grid mx-1 grid-cols-2 mt-8 '>
                        ...
                        <div className='mx-auto w-full'>
                            ...
                            <select value={paymentTerms} onChange={(e) => setpaymentTerms(e.target.value)} className=' appearance-none w-full py-2 px-4 border-[.2px] rounded-lg focus:outline-none dark:bg-[#1e2139] dark:text-white dark:border-gray-800 focus:outline-purple-400 border-gray-300 select-status' >
                                {deliveryTimes.map(time => (<option value={time.value}>{time.text}</option>))}
                            </select>
                        </div>
                    </div>
